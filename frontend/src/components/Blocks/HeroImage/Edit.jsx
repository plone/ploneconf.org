...
class EditComponent extends Component {
  ...
  constructor(props) {
    ...
    if (!__SERVER__) {
      ...
      const blockSubTitleRenderMap = Map({
        unstyled: {
          element: 'h3',
        },
      });
      ...
      this.extendedSubTitleBlockRenderMap = DefaultDraftBlockRenderMap.merge(
        blockSubTitleRenderMap,
      );
      ...
      if (props.data && props.data.subTitle) {
        subTitleEditorState = EditorState.createWithContent(
          stateFromHTML(props.data.subTitle),
        );
      } else {
        subTitleEditorState = EditorState.createEmpty();
      }
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.data.subTitle &&
      this.props.data.subTitle !== nextProps.data.subTitle &&
      !this.props.selected
    ) {
      const contentState = stateFromHTML(nextProps.data.subTitle);
      this.setState({
        subTitleEditorState: nextProps.data.subTitle
          ? EditorState.createWithContent(contentState)
          : EditorState.createEmpty(),
      });
    }
  ...
  /**
   * Change Sub Title handler
   * @method onChangeSubTitle
   * @param {object} subTitleEditorState Editor state.
   * @returns {undefined}
   */
  onChangeSubTitle(subTitleEditorState) {
    this.setState({ subTitleEditorState }, () => {
      this.props.onChangeBlock(this.props.block, {
        ...this.props.data,
        subTitle: subTitleEditorState.getCurrentContent().getPlainText(),
      });
    });
  }
              ...
              <Editor
                ...
                onDownArrow={() => {
                  ...
                  if (
                    ...
                    this.setState(() => ({ currentFocused: 'subTitle' }));
                    this.subTitleEditor.focus();
                  }
                }}
              />
              <Editor
                ref={(node) => {
                  this.subTitleEditor = node;
                }}
                readOnly={!this.props.editable}
